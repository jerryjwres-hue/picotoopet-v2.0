...
/// <summary>业务自动化事实页；串联 Business → Pipeline → Creative → Production → Deep-AI → Evaluation → Shadow。</summary>
public sealed class BusinessAutomationPageViewModel : PageViewModel
{
    ...
    public BusinessWorkPackageRecord? SelectedPackage
    {
        ...
        set
        {
            if (SetProperty(ref _selectedPackage, value))
            {
                ...
                // Project scope bridge       Evaluation/Shadow receive only the trusted selected-package project identity.
    ...
    /// <summary>刷新固定 Inbox/Outbox 与完整 Business → Creative → Production → Deep-AI → Evaluation → Shadow 控制面。</summary>
    public async Task RefreshAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await Task.WhenAll(
                ...
                deepAiTask,
                evaluationTask,
                shadowTask).ConfigureAwait(false);
            StatusMessage =
    ...
    public async Task SubmitInboxAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await Task.WhenAll(
                ...
                QualityShadow.RefreshAsync(cancellationToken)).ConfigureAwait(false);
