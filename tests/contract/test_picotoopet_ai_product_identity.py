...
def test_windows_identity_is_picotoopet_ai_superpower_v1() -> None:
    ...
    assert "ControlCenterSubtitle" in version_text

    assert 'xmlns:versioning="clr-namespace:PicotooPet.Desktop.Versioning"' in shell_text
    assert 'Text="{x:Static versioning:ProductVersionInfo.ProductName}"' in shell_text
    assert 'Text="{x:Static versioning:ProductVersionInfo.SuperpowerLabel}"' in shell_text
