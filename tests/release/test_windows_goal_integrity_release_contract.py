...
def test_windows_release_stamper_declares_native_wpf_goal_fields() -> None:
    ...
    required_contract_values = (
        '"source_build_on_user_pc": false', '"delivery_surface": "existing-native-wpf-desktop"',
        ...
        '"product_version"', '"value": "2.3.26.1"',
    )
    # Release gate              Stamping/verifying must bind the current 26.1 native-WPF goal.
...
def test_windows_release_runs_goal_integrity_gate_before_artifact_upload() -> None:
    ...
    assert "goal-integrity-report.json" in workflow and "project-goal-integrity-report.json" in workflow
...
def test_windows_release_does_not_name_or_package_a_helper_surface() -> None:
    ...
    for value in ("prebuilt-helper", "browser-localhost-helper", "picotoopet slice d.exe", "slicedhelper", "windows-helper"):
        assert value not in builder and value not in workflow
