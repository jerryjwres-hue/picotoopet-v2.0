...
def test_migration_eight_creates_commit_candidate_table_idempotently(tmp_path: Path) -> None:
    """Migration 8 必须幂等创建 Commit Candidate 安全事实表。"""
    ...
    columns = {
        row["name"]
        for row in database.fetchall("PRAGMA table_info(provider_commit_candidates)")
    }
...
def test_migration_eight_preserves_existing_adoption_rows(tmp_path: Path) -> None:
    """Commit Candidate 迁移不得改写历史 Adoption Candidate。"""
