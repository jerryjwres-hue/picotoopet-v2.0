...
REQUIRED_CONFIRMATION_COLUMNS = {
    "confirmation_id", "handoff_id", "provider", "status", "request_digest", "package_digest",
    "budget_json", "idempotency_key", "confirmed_at", "expires_at", "preview_json",
}
REQUIRED_PROVIDER_SESSION_COLUMNS = {
    "session_id", "handoff_id", "provider", "status", "request_digest", "package_digest",
    "budget_json", "turns_used", "elapsed_seconds", "changed_file_count", "return_id", "failure_code",
    "provider_usage_unknown", "idempotency_key", "created_at", "updated_at", "finished_at", "preview_json",
}
...
def test_migration_six_creates_provider_fact_tables_idempotently(tmp_path: Path) -> None:
    ...
    confirmation_columns = {row["name"] for row in database.fetchall("PRAGMA table_info(provider_usage_confirmations)")}
    ...
    # Schema retention gate      Migration 6 remains exactly once inside cumulative schema 21.
...
def test_migration_six_preserves_existing_handoff_return_and_broker_rows(tmp_path: Path) -> None:
    database = Database(tmp_path / "upgrade.db")
    database.open(); _install_first_five(database)
    now = "2026-08-07T03:00:00+00:00"; expires = "2026-08-07T04:00:00+00:00"
    database.execute(
        "INSERT INTO handoffs VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, NULL, ?, ?, ?)",
        ("handoff-existing", "picotoopet-repo-maintenance-v1", "existing", "existing objective", "approved", "a" * 64, "b" * 64, "{}", "{}", "prepare-existing", now, now, expires),
    )
    database.execute(
        "INSERT INTO returns VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, ?, ?)",
        ("return-existing", "handoff-existing", "contract_validated", "local-contract-self-test", "a" * 64, "b" * 64, "c" * 64, 0, 3, "[]", "{}", "return-key-existing", now, now),
    )
    database.execute(
        "INSERT INTO broker_sessions VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, ?, ?, ?, ?)",
        ("broker-existing", "handoff-existing", "completed", "local-mock-dev-broker", 30, "a" * 64, "b" * 64, "return-existing", 4, "d" * 64, "broker-key-existing", now, now, now, "{}"),
    )
