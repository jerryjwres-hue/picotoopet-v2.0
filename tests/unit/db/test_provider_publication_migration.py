...
def test_migration_ten_creates_publication_candidate_table_idempotently(tmp_path: Path) -> None:
    ...
    # Schema retention gate      Migration 10 remains once and later migrations advance through 23.
    ...
    assert database.scalar("SELECT COUNT(*) FROM schema_migrations WHERE version = 23") == 1
    database.close()
