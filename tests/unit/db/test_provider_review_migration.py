...
def test_migration_seven_creates_review_and_adoption_tables_idempotently(
    tmp_path: Path,
) -> None:
...
def test_migration_seven_preserves_existing_provider_session_rows(tmp_path: Path) -> None:
    ...
    database.open()
    database.apply_migrations()
    now = "2026-08-07T15:20:00+00:00"
    expires = "2026-08-07T16:20:00+00:00"
    database.execute(
        "INSERT INTO handoffs VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, NULL, ?, ?, ?)",
        (
            "handoff-review-existing",
    ...
    before = database.fetchone(
        "SELECT handoff_id, status, request_digest, package_digest FROM provider_sessions "
        "WHERE session_id = ?",
        ("11111111-1111-1111-1111-111111111111",),
    )
    database.apply_migrations()
