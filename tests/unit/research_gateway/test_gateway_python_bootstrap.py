...
def test_old_python_resolves_only_adapter_private_compatible_runtime(tmp_path: Path) -> None:
    ...
    candidate = gateway._adapter_bootstrap_python(
        version_info=(3, 9),
        environ={"PICOTOOPET_CRAWL4AI_ROOT": str(adapter_root)},
        home=tmp_path / "home",
    )
    assert candidate == private_python


def test_supported_python_does_not_reexec_even_when_private_runtime_exists(
    tmp_path: Path,
) -> None:
    adapter_root = tmp_path / "crawl4ai"
    private_python = adapter_root / "venv" / "bin" / "python"
