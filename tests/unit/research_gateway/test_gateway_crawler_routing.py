...
def test_research_search_keeps_public_capability_and_enriches_with_typed_crawl_content() -> None:
    search_payload = json.dumps({"results": [{"title": "Example search hit", "url": "https://example.com/article"}]})
    runner = RecordingRunner(CommandResult(returncode=0, stdout=search_payload, stderr=""))
    crawler = RecordingCrawler(CrawlerDocument(title="Example Article", url="https://example.com/article", source="example.com", markdown="# Example Article\n\nClean body.", provider=CrawlerProvider.CRAWL4AI, status_code=200))
    ...
    assert envelope["documents"] == [{"markdown": "# Example Article\n\nClean body.", "provider": "crawl4ai", "source": "example.com", "status_code": 200, "title": "Example Article", "url": "https://example.com/article"}]
...
def test_research_search_returns_controlled_failure_when_selected_url_cannot_be_crawled() -> None:
    search_payload = json.dumps({"results": [{"url": "https://example.com/article"}]})
    runner = RecordingRunner(CommandResult(returncode=0, stdout=search_payload, stderr=""))
    crawler = RecordingCrawler(CrawlerProviderError("all crawler providers failed", provider=CrawlerProvider.SCRAPLING))
    gateway = GatewayDispatcher(runner=runner, crawler_adapter=crawler)
    result = gateway.dispatch("research.search", {"query": "example", "limit": 5})
