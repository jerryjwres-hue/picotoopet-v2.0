...
def test_supervisor_aggregates_database_disk_and_resident_health(tmp_path: Path) -> None:
    """健康监督器必须检查数据库、磁盘和核心模型并持久化结果。"""
    ...
    supervisor = HealthSupervisor(database=database, paths=paths, resident=resident)
    ...
    assert database.scalar("SELECT COUNT(*) FROM service_health") == 3
    database.close()
