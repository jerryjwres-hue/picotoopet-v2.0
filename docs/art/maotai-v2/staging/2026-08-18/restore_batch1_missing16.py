#!/usr/bin/env python3
"""Restore the Maotai v2 2026-08-18 missing-16 restart checkpoint.

The sibling .mtr.b85 file is a compact, reversible logical-resolution checkpoint.
It restores RGBA candidate PNGs only; runtime promotion must still apply the
frozen MaotaiAssetManifest @2x dimensions/pivots and run the formal pixel gate.
"""
...
if __name__ == "__main__":
    ...
    pack = Path(sys.argv[1]) if len(sys.argv) > 1 else here / "batch1_missing16.mtr.b85"
    out = Path(sys.argv[2]) if len(sys.argv) > 2 else here / "batch1_restored"
    for path in restore(pack, out):
        print(path)
